...
VOID DlgPlayList::addSongToPlayList(HSTREAM stream, TCHAR*path)
{
	...
	/*
		Заполнение структуры (Информация о песне)
	*/
	if (id3 == NULL)
	{
		strcpy(info.title, "Название ");						//Название
		strcpy(info.artist, "Исполнитель ");					//Исполнитель
		strcpy(info.album, "Альбом ");							//Альбом
		strcpy(info.year, " Год");								//Год
	}
	else
	{
		strcpy(info.title, id3->title);							//Название
		strcpy(info.artist, id3->artist);						//Исполнитель
		strcpy(info.album, id3->album);							//Альбом
		strcpy(info.year, id3->year);							//Год
	}
	
	info.minutes = min;														//Минуты
	info.seconds = seconds;													//Секунды
	info.hStream = stream;													//Поток
	lstrcpy(info.path, path);												//Путь к песне
	...
	ListBox_AddString(hPlayList, infoAboutTheSong.str().c_str());			//Добавление в PlayList
	songs.push_back(info);													//Добавление в вектор 
}
/*
